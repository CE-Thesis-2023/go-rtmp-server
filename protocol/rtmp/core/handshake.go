...
func (conn *Conn) HandshakeClient() (err error) {
	var random [(1 + 1536*2) * 2]byte // 6146 -> this is an array of byte with size of 6146

	C0C1C2 := random[:1536*2+1] // 0->3073
	C0 := C0C1C2[:1] // 0
	C0C1 := C0C1C2[:1536+1] // 0 -> 1537
	C2 := C0C1C2[1536+1:] //1537 -> 3073

	S0S1S2 := random[1536*2+1:] // 3073 -> 6146

	C0[0] = 3 // first value is 3
	// > C0C1 
	conn.Conn.SetDeadline(time.Now().Add(timeout)) // set time out for the connection
