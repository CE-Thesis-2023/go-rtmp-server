...
func (s *Server) Serve(rtmpListener net.Listener) (err error) {
	//The surrounding function somehow panic/stop running -> this defer function will run
	defer func() {
		if r := recover(); r != nil {
...
func (s *Server) handleConn(conn *core.Conn) error {
	...
	appname, name, _ := connServer.GetInfo() //rtmp://localhost:1935/{appname}/channelKey
